...
Qitmeer has a big picture on blockchain based Islamic Finance. The scale of which is enormous, thus it brings a tensive challenge to the design of technology architect. That is why the design philosophy is critical to the project.

\subsubsection{Respect classic}

Bitcoin has various problems so far, like limited scalability,  high risk of centralization due to monopolistic mining pools. However, most of its mechanisms are a long time proved robust and promising. So Qitmeer inherits most of its classical designs, like UTXO,  Proof of work.

\subsubsection{Embrace innovation}

Although there is an enormous number of solutions to claim to have solved current challenges, such as the risk of centralization, low scalability, long confirmation, etc., few of them indeed have solved all, mostly just a small set of those problems. In order to reach the stringent Classical Blockchain Setting, Qitmeer adopts several new technologies, e.g., leadingly implements SPECTRE and PHANTOM protocols,  which are still in the academic stage.

\subsubsection{Simple and extensible}
The Islamic financial scenarios are diverse, so it will become extremely unmaintainable to devise a single complicated system to cover them all. Qitmeer's architecture is a multiple layered stack, and each layer is as simple as possible to focus on just a core problem. For instance, the public chain is merely a simple UTXO based value transfer network, and there only are off-chain smart contracts. Another case is Qitmeer using Block DAG to gain sufficient scalability, Block DAG, working in ledger layer, can correctly be extended with Sharding and off-Chain to support further scalability because they work on different layers.
