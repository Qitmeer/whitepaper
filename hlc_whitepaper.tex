...
Throughput is a representative metric of scalability, also it is a strong factor affecting practical scenarios. Bitcoin and ethereum don't scale, which have only 7 TPS and 15 TPS in theory. As a global value transfer network, such low throughput limits the variety of applications. Qitmeer is using Block DAG as our ledger underlying data structure and it could provide a conservative estimation of at least one thousand transactions per second throughput to meet most applications on blockchain so far.

\subsection{Methodology}

Qitmeer has a big picture on blockchain based Islamic Finance. The scale of which is huge, thus it brings a tensive challenge to the design of technology architect. That is why the design phylosophy is critical to the project:

\subsubsection{Respect classic}

Bitcoin has various challenges so far, like limited scalability,  high centralization due to monopolistic mining pools. However, most of its mechanisms are long time proved robust and promising. So Qitmeer inherits most of its classical designs, like UTXO,  Proof of work.

\subsubsection{Embrace innovation}

Although there are an enormous number of solutions to clam to have solved current challenges, such as high centralization, low scalability, long confirmation, etc., few of them really have solved all, mostly just a small set of those problems. Qitmeer proposes a stringent blockchain metrics called Classical Blockchain Setting, to reach this metrics, Qitmeer would like to take risks in adoption of new technologies, even they are not verifiable enough by the market. So, Qitmeer leadingly implemented SPECTRE and PHANTOM protocols,  which are still in academic stage.

\subsubsection{Simple and extensible}
The Islamic financial scenarios is diverse, so it will become extremely unmaintainable to devise a single complicated system to cover them all. Qitmeer's architecture is a mulitple layered stack, each layer is as simple as possible to focus on  just a core problem. For instance, the public chain is merely a simple UTXO based value transfer network and there only are off-chain smart contracts. Another case is Qitmeer using Block DAG to gain sufficient scalability, Block DAG, working in ledger layer,  is able to perfectly extended with Sharding and off-Chain to support further scalability because they work on different layers.
